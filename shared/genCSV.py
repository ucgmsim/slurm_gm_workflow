...
def subsurface_general():
    data = []
    # add header
    data.append(["#site_amp_model","flowcap","fmid","fmidbot","fmin","fhigh","fhightop","fmax","dt","ft_len"])
    # get data
    data.append([site_amp_model,site_flowcap,site_fmid,site_fmidbot,site_fmin,site_fhigh,site_fhightop,site_fmax,dt,hf_t_len])
...
def subsurface_per_station():
    ...
    for station in station_values.keys():
        ...
        for_output = [station, dict_values["000"], dict_values["090"], dict_values["ver"], dict_values["vs_ref"], dict_values["vs_30"]]
...
def gen_rup_segments(srf_files):
    #initialize csv
    segments = []
    segments.append( ('#rup','depth','lat','lon','f_len','f_wid','d_top','s_hyp','d_hyp','stk','dip','rak') )
    ruptures = []
    ruptures.append( ('#NAME(unique)','short_name','DT','R_TYPE','PATH') )
    planes_list = []
    rup_names = []
...
#        print "cannot open stat file: %s"%stat_file
    ...
    #initialize csv
    statgrid = []
    statgrid.append( ("#NAME","CODE(unique)","REAL(0-True,1-False)","LATITUDE","LONGITUDE","LOCATION_GIS(empty)") )
...
def gen_event(location, datetime, exetime):
    ...
    event.append(('#LOCATION', 'MAGNITUDE', 'DATE TIME (IN YYYY-MM-DD HH-MM-SS FORMAT)','PATH','NAME (unique-COMBINE LOCATION&MAGNITUDE&DATE TIME)' ))
