...
#execfile(params_vel,globals()) #no longer needed: all processed and added to params_base.py
#execfile(params_src,globals()) #appears unnecessary


# number of processes
n_proc = '512'  
# low pass frequency, filter for output seimograms
#flo = '1.0' # hertz  #available in params_base.py

######## Global CONSTANTS ########
# dt should be 0.005 (or smaller), small increments required in simulation
#this value is not located in params_base.py
#dt = 0.005
...
nt = str(int(round(float(sim_duration)/float(dt))))## cap number of timesteps in simulation, not all timesteps have outputs
...
wcc_prog_dir = os.path.join(global_root, tools_dir)
#seis_tmp_dir = os.path.join(user_scratch, run_name, 'SeismoBin') #written directly to e3d.par
...
hf_sim_bin = os.path.join(global_root, os.path.join(tools_dir,'hb_high_v5.4.5_np2mm+'))
hf_prefix = 'hf'
# duration of HF sim
hf_t_len = sim_duration #'100' # seconds
# HF simulation step. should be small
# this is moved to params_base.py
# hf_dt = 0.005 # '0.005' # seconds
# slip model
# 1D velocity model
...
match_hf_fhi = flo        # high pass frequency, Hz
...
match_lf_flo = flo        # low pass frequency, Hz
...
# use power based matched filter
match_powersb = True
...
GEOPROJ = '1'
# binaries
XYZ2LL_BIN = os.path.join(global_root, os.path.join(tools_dir, 'xyz2lonlat') )
LL2GRID_BIN = os.path.join(global_root, os.path.join(tools_dir,'latlon2statgrid') )
# station file list for generating station grid
#STAT_FILES = [stat_file] #now to be found in params_base.py

STATGRID_GEN = 'statgrid-%sx%s%s.ll' %(float(hh)*float(dx_ts),float(hh)*float(dy_ts),sufx) # output statgrid file
