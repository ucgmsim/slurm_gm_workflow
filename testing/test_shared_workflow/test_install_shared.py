...
def test_install_simulation(set_up, mocker):
    ...
    for root_path, _ in set_up:
        ...
        mocker.patch('shared_workflow.install_shared.defaults.recipe_dir', os.path.join(root_path, '..', '..', 'templates'))

        for i in range(len(input_params)):
            if isinstance(input_params[i], str) and input_params[i].startswith(('CSRoot', 'AdditionalData', 'PangopangoF29/')):
