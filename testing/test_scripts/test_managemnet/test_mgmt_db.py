...
def test_force_update_db(mgmt_db):
    update_mgmt_db.force_update_db(mgmt_db, TEST_PROC[1], FORCE_STATUS[1], run_name=TEST_RUN_NAME)
    value = get_rows(mgmt_db, 'state', 'proc_type', TEST_PROC[0], selected_col='status')[0][0]
    assert value == FORCE_STATUS[0]


def test_force_update_db_error(mgmt_db):
    update_mgmt_db.force_update_db(mgmt_db, TEST_PROC[1], FORCE_STATUS[1], run_name=TEST_RUN_NAME, retry=True,
                                   reset_retries=True, error='another error')
