...
def get_lines(stat_list_file, out_path, sample_size):
    ...
    if len(old_output_samples) == 0:
        ...
        old_outputs_bad = [
            os.path.splitext(os.path.basename(x))[0] for x in old_outputs_bad
        ]  # just take the filename, no path, no extension name

        stations_completed = [
            os.path.splitext(os.path.basename(x).strip("hf_"))[0]
            for x in old_outputs_good
        ]
    ...
    with open(stat_list_file, "r") as fp:
        for line in fp.readlines():
            ...
            try:
                ...
                print("Bad entry: %s" % line_split, file=sys.stderr)
    ...
    for s in stations_completed:
        try:
            ...
            print("Bad key: %s" % s, file=sys.stderr)
...
def get_codes(stations_all, vel_path):
    ...
    if len(old_outputs) == 0:
        ...
        old_outputs_good = [v for v in old_outputs if os.stat(v).st_size == normal_size]
        stations_completed = [
            os.path.splitext(os.path.basename(x))[0] for x in old_outputs_good
        ]
        print(">>> Already done for %d stations" % len(stations_completed))
        print(stations_completed)
