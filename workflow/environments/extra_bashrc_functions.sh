...
activate_env () {
    ...
    source ${env_path}/workflow/environments/helper_functions/activate_env.sh ${env_path} ${hpc}
}

deactivate_env () {
    if [[ ! -z ${CUR_ENV} ]]; then
        source ${CUR_ENV}/workflow/environments/helper_functions/deactivate_env.sh
    else
