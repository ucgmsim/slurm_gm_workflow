...
if rank == 0:
    processed_stats=[]
    for processed_stats_sublist in processed_stats_all_container:
        processed_stats.extend(processed_stats_sublist)

    print stats
    print processed_stats
    stats_left= list(set(stats)-set(processed_stats))
    stats_left.sort()
    if len(stats_left) > 0:
