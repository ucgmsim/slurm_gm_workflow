...
def q_select_rupmodel_dir(mysrf_dir):
    ...
    srf_options = [x for x in srf_options if os.path.isdir(os.path.join(mysrf_dir,x)) and not x.startswith('.')] #avoid displaying .git
    srf_options.sort()
    if len(srf_options)==0: #this is a dead end
        print "No valid ruptuer model found"
        sys.exit()
...
def action(sim_dir,event_name, run_name, run_dir, vel_mod_dir,srf_dir, srf_stoch_pairs,params_vel_path,stat_file_path, vs30_file_path, vs30ref_file_path, MODEL_LAT,MODEL_LON,MODEL_ROT,hh,nx,ny,nz,sufx,sim_duration,flo,vel_mod_params_dir,yes_statcords,yes_model_params):
...
def get_input_wc():
    show_horizontal_line()
    try:
        user_input_wc = datetime.datetime.strptime(str(raw_input("Enter the WallClock time limit you will like to use: ")), "%H:%M:%S").time()
    except ValueError:
        wc_usage()
...
#    vel_mod_params_dir = os.path.join(global_root, "VelocityModel/SthIsland/ModelParams")
    ...
    event_name = ""
    action(sim_dir,event_name, run_name, run_dir, vel_mod_dir_full,      srf_dir, srf_stoch_pairs,params_vel_path,stat_file_path, vs30_file_path, vs30ref_file_path, MODEL_LAT,MODEL_LON,MODEL_ROT,hh,nx,ny,nz,sufx,sim_duration,flo,vel_mod_params_dir,yes_statcords, yes_model_params)
...
#    action(sim_dir,event_name,run_name,vel_mod_dir, srf_stoch_pairs,params_vel_path,stat_file_path, vs30_file_path, vs30ref_file_path, MODEL_LAT,MODEL_LON,MODEL_ROT,hh,nx,ny,nz,sufx,sim_duration,flo,vel_mod_params_dir,yes_statcords, yes_model_params)
    srf_dir = srf_default_dir #the above is perhaps unnecessary
    action(sim_dir,event_name,run_name, run_dir, vel_mod_dir,      srf_dir, srf_stoch_pairs,params_vel_path,stat_file_path, vs30_file_path, vs30ref_file_path, MODEL_LAT,MODEL_LON,MODEL_ROT,hh,nx,ny,nz,sufx,sim_duration,flo,vel_mod_params_dir,yes_statcords, yes_model_params)
#   
    print "Installation completed"
    show_instruction(sim_dir)
