...
if [[ $exit_val == 0 ]]; then
    ...
    for f in $OUTPUT_XYZ_DIR/*.xyz; do
        ...
        cd $out_dir
        if [[ "$f" == *"real"* ]]; then
          res2=$(python $gmsim/visualization/visualization/gmt/plot_items.py -t $SRF_NAME --xyz $f -c "$SRF_PATH" --xyz-cpt hot --xyz-cpt-invert --xyz-transparency 30 --xyz-size 0.1 --xyz-cpt-labels `cat im_order.txt`)
        else
          res2=$(python $gmsim/visualization/visualization/gmt/plot_items.py -t $SRF_NAME --xyz $f -c "$SRF_PATH" --xyz-model_params $MODEL_PARAMS --xyz-grid --xyz-grid-search 12m --xyz-landmask --xyz-cpt hot --xyz-cpt-invert --xyz-grid-contours --xyz-transparency 30 --xyz-size 1k --xyz-cpt-labels `cat im_order.txt`)
        fi
        exit_val2=$?
        if [[ $exit_val2 != 0 ]]; then
            failed=1
