...
if __name__ == "__main__":
    ...
    def unfinished(out_file):
        ...
        if np.min(checkpoints):
            try:
                ...
                raise RuntimeError("HF Simulation already completed.")
    ...
    vm = args.velocity_model
    for s in range(stations_todo.size):
        if args.site_vm_dir != None:
    ...
    os.remove(in_stats)
    print("Process finished (%.2fs)." % (time() - t0))
    logger.debug(
        "Process completed {} stations ({:.2f}).".format(
            stations_todo.size, time() - t0
        )
    )
