...
python $gmsim/workflow/scripts/cybershake/add_to_mgmt_queue.py {{mgmt_db_location}}/mgmt_db_queue {{srf_name}} 11 3
...
if [[ $exit_val == 0 ]]; then
    #passed
    python $gmsim/workflow/scripts/cybershake/add_to_mgmt_queue.py {{mgmt_db_location}}/mgmt_db_queue {{srf_name}} 11 4
#    echo "python $gmsim/workflow/scripts/management/update_mgmt_db.py $MGMT_DB_LOC clean_up completed --run_name $SRF_NAME --force " >> $MGMT_DB_LOC/mgmt_db_queue/$timestamp\_$SLURM_JOBID
    ...
    python $gmsim/workflow/scripts/cybershake/add_to_mgmt_queue.py {{mgmt_db_location}}/mgmt_db_queue {{srf_name}} 11 5
