...
def create_sl(bb_sim_dirs,sl_template_prefix,submit_yes):
    ...
    #    variation = '_'.join(bb_sim_dir.split('/')[0:2])
        ...
        memory = "16G"
        header = resolve_header("nesi00213", nb_cpus, run_time, job_name, bin_process_ver, memory,job_description="BB calculation", additional_lines="#SBATCH -C avx")
        f_slscript.write(header)
        f_slscript.write('# script version: %s\n'%bin_process_ver)
        f_slscript.write(txt)
        f_slscript.close()
        print "Loadleveler script %s written" %fname_slscript
        if submit_yes:
            print "Submitting %s" %fname_llscript
            #TODO:
            #change this to call submit_sl_script()
