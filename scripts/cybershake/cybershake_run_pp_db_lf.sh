...
if [ ! -f $run_dir/../pp_completed_lf.log ]; then
    touch $run_dir/../pp_completed_lf.log
fi
#read the completed pp list
completed_pp_list=`cat $run_dir/../pp_completed_lf.log`
...
do
    ...
    for i in `find $run_dir/$run_name -name "pp_config_db_lf.cfg"`;
    do
    ...
    echo "$run_name" >> $run_dir/../pp_completed_lf.log
done
